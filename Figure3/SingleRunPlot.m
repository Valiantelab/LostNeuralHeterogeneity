function runs_split=SingleRunPlot(target, sigmae, sigmai)
close all

%% Import Data
filename1=sprintf('SpikingNEWNEW_p100_sige%1.0f_sigi%1.0f_Trials10_gamma 16.csv', sigmae*10, round(sigmai*10));
filename2=sprintf('SpikingInhibNEWNEW_p100_sige%1.0f_sigi%1.0f_Trials10_gamma 16.csv', sigmae*10, round(sigmai*10));
...
%             plot(start:1:(endtime-window), (synchrony(k,:)./.35)*1000, 'Color', 'r', 'LineStyle', '-', 'LineWidth', 3, 'Marker', 'none');
        elseif sigmae==7.8
            plot(start:1:(endtime-window), (synchrony(k,:)./.45)*1000, 'Color', 'b', 'LineStyle', '-', 'LineWidth', 3, 'Marker', 'none');
...
%         set(gca, 'YTickLabel', [0 0.175 0.35]);
        ...
        set(gca, 'FontSize', 24);
        axis([0 endtime 0 25])
%         axis([0 1000 0 max([firingrate(k,1:1000), firingrate_i(k,1:1000)])+1])
        set(gca, 'XTickLabel', []);
        set(gca, 'FontSize', 24);
